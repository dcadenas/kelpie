...
daemon parks Herdr I/O, readiness checks, retries, and prompt-spacing waits, so
one waiting request does not delay unrelated clients. A `start` or other parked
operation may answer after the caller's own timeout; the operation continues to
its durable outcome even if that caller disconnects. The typed client persists
every received response in `last-response.ndjson`.
...
`inbox.ack`. A delivery line is complete only at the newline; keep buffering
until it arrives. If the connection ends mid-line, discard the fragment —
reconnect re-offers that queued row. Dropping the connection leaves those
...
- `tell` accepts the same exact Herdr target, or `recipient_alias` resolved once
  to the unique active delivery target for that public name. A `herdr_prompt`
  target binds its exact Ready incarnation; a `socket_inbox` target binds the
...
the incarnation is `idle` or `done`. For a one-shot it is the wall-clock time
written when that cycle was armed. For a cycle already in flight it is the
instant that cycle left `scheduled`, already past. The client renders
`next-in=` only for a scheduled cycle for that reason. `renew_id` names the
cycle, not the rule: a standing policy mints a new id for each successor, so
continuity is `every_ms` plus a rising `cycle`, not a stable id.
