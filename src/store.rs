...
/// ID atomically created for a tell queued to a pane-less socket waiter.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct CreatedSocketTell {
    pub message_id: MessageId,
}
...
impl Store {
    ...
    /// Return aliases that a new Herdr-bound identity may not claim.
    ///
    /// Ready incarnations and active socket waiters are both live delivery
    /// targets. Name derivation must see both before choosing its stable
    /// pane-suffixed fallback; otherwise it proposes the base name and only
    /// discovers the socket-waiter collision while committing the adoption.
    pub fn active_aliases(&self) -> Result<Vec<String>, StoreError> {
    ...
    /// Resolve one active socket waiter by public alias.
    ///
    /// Historical ended waiters do not participate. More than one active
    /// claimant is an invariant violation and fails closed.
    ///
    /// # Errors
    ///
    /// Returns store errors or a conflict for multiple active claimants.
    pub fn active_socket_waiter_for_alias(
        &self,
        public_name: &str,
        ...
        match ids.as_slice() {
            [] => Ok(None),
            [id] => Ok(Some(parse_logical_agent_id(id)?)),
            _ => Err(StoreError::Conflict(format!(
                "alias {public_name} is held by {} active socket waiters",
                ids.len()
            ))),
        }
    }
    ...
    /// Atomically persist a tell to an active socket waiter's durable inbox.
    ///
    /// The delivery stays `queued` until that waiter ACKs it. A future due time
    /// delays when a connected inbox may first be offered the row.
    ///
    /// # Errors
    ///
    /// Returns a conflict for an absent sender or inactive waiter, or a store
    /// error if the atomic message, delivery, and idempotency write fails.
    pub fn create_socket_tell(
        &mut self,
        sender: LogicalAgentId,
        recipient: LogicalAgentId,
        body: &str,
        ...
        let schedule = delivery_schedule(now, due_at_ms)?;
        let tx = self.connection.transaction()?;
        require_active_socket_waiter(&tx, recipient)?;
        let sender_exists: bool = tx.query_row(
            "SELECT EXISTS(SELECT 1 FROM logical_agents WHERE id = ?1)",
            [sender.to_string()],
            |row| row.get(0),
    ...
    fn name_info_on(conn: &Connection, public_name: &str) -> Result<NameInfo, StoreError> {
        ...
        for row in rows {
            ...
            claimants.push(NameClaimant {
                logical_agent_id,
                created_at_ms,
                delivery_transport: parse_delivery_transport(&transport)?,
    ...
    pub fn accrue_renew_occupancy(
        ...
        tx.execute(
            "UPDATE schedules
             SET active_remaining_ms = (SELECT active_remaining_ms FROM renews WHERE id = ?1),
                 occupancy_sampled_at_ms = ?2,
                 next_fire_at_ms = (SELECT scheduled_at_ms FROM renews WHERE id = ?1)
             WHERE id = (SELECT schedule_id FROM renews WHERE id = ?1)
               AND state = 'active'",
...
fn arm_next_renew_cycle(
    ...
    let policy: Option<(Option<i64>, i64, Option<String>)> = tx
        ...
        .optional()?;
    let Some((Some(every_ms), cycle, Some(schedule_id))) = policy else {
        return Ok(None);
    };
    let active: bool = tx.query_row(
        "SELECT EXISTS(SELECT 1 FROM schedules WHERE id = ?1 AND state = 'active')",
        [&schedule_id],
        |row| row.get(0),
    )?;
    if !active {
...
fn migrate(connection: &Connection) -> Result<(), StoreError> {
    ...
    if version == 23 {
        connection.execute_batch(include_str!("../migrations/024_reminder_scan_indexes.sql"))?;
        version = 24;
    }
    if version == 24 {
...
fn queue_socket_inbox_delivery(
    ...
    queue_socket_inbox_delivery_at(tx, message_id, recipient_agent_id, now)
}

fn queue_socket_inbox_delivery_at(
    tx: &Transaction<'_>,
    message_id: MessageId,
    recipient_agent_id: LogicalAgentId,
    scheduled_at_ms: i64,
...
mod tests {
    ...
    #[test]
    fn reminder_scan_indexes_are_installed() {
    ...
    fn create_new_refusal_names_the_asks_both_parties_and_three_remedies() {
        let info = NameInfo {
            ...
            claimants: vec![NameClaimant {
                ...
                created_at_ms: 1,
                delivery_transport: DeliveryTransport::HerdrPrompt,
    ...
    #[test]
    fn socket_waiter_alias_resolves_and_accepts_an_unsolicited_tell() {
        let mut store = Store::in_memory().expect("store");
        let sender = store
            .declare_start(&intent("sender", "term-sender", "sender-start"))
            .expect("sender");
        ...
        assert_eq!(
            store
                .active_socket_waiter_for_alias("botserver")
                .expect("resolve waiter"),
            Some(waiter.logical_agent_id)
        );
        let info = store.name_info("botserver").expect("name info");
        assert_eq!(info.claimants.len(), 1);
        assert_eq!(
            info.claimants[0].delivery_transport,
            DeliveryTransport::SocketInbox
        );
        assert!(!info.claimants[0].has_ready_incarnation);
        assert!(info.claimants[0].is_addressable);

        let tell = store
            .create_socket_tell(
                sender.logical_agent_id,
                waiter.logical_agent_id,
                "unsolicited progress",
                "socket-tell",
                None,
            )
            .expect("tell");
        ...
        assert_eq!(
            store
                .delivery_outcome_for_message(delayed.message_id)
