...
pub(crate) fn parse_agent_result(result: &Value) -> Result<AgentObservation, HerdrError> {
    let agent = result
        .get("agent")
...
pub(crate) fn request_over_stream(
    mut stream: &mut (impl Read + Write),
    id: &str,
    method: &str,
    params: &Value,
    pause_after_write: bool,
    on_written: Option<&dyn Fn()>,
    ...
    if pause_after_write && let Some(point) = after_write_fault_point(id, method) {
