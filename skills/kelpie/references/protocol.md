...
- `waiter.register`: create a pane-less LogicalAgent with socket-inbox delivery.
  ...
  `message_id` is create order, not offer order. Kelpie offers every due
  unACKed row; an ACKed row is never re-offered, so a host needs no cursor. A
  lower id after a higher one is a delayed tell arriving when due.
  `inbox.delivery` includes `scheduled_at_ms` and `created_at_ms` for
  staleness, not dedup. `pending` and `ask.info` are not the socket-waiter
  receive path. Kelpie
