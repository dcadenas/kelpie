...
- Keep one idempotency key for one intended prompt. Repeating it after success
  returns the recorded receipt; repeating it after a proven terminal failure
  starts a fresh attempt. Pending, accepted, superseded, and unknown outcomes
  are refused with the prior outcome named. Never vary the key to bypass that
  refusal, because the original effect may already have landed.
