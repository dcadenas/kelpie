...
- `tell` informs. `delivery=accepted` means the prompt was submitted. It does
  not mean they will write back. `pending` will not list a tell.
...
agent. If that pane, terminal, and backend already have a unique lost or unknown
incarnation, the adoption continues that logical agent and keeps its recorded
alias. An unnamed occupant is renamed back to that alias; a different live name,
a backend mismatch, or several continuable agents fail closed and demand
`adopt --logical-id`. A missing recipient alias may likewise adopt one unique
unnamed live agent whose cwd basename derives to that alias. Ambiguity fails
closed.
...
- `tell`: deliver a structured one-way message. Provide either exact
  `recipient` + `recipient_incarnation`, or `recipient_alias` (public name
  resolved once at send time to the unique Ready agent). An optional due time
  persists the delivery as `queued` and fires it once when due against that
  exact Ready incarnation. This is delayed one-way delivery, not a schedule.
