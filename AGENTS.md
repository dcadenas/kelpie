...
- `src/bin/kelpied.rs` → `daemon.rs` — foreground daemon. Opens and recovers the
  ...
  `schedule.cancel`, `schedule.list`, `reminder.snooze`, `reminder.disable`,
  `notice.create`, `notice.list`, `who`, `replies.claim`, `replies`, `replies.ack`, and the legacy identity aliases). `inbox.claim` is the exception: it
