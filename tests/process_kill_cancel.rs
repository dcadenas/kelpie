...
//!
//! One cancel prompts two panes, the waiting agent and the owing agent, and
//! the daemon opens both Herdr connections from per-pane executor lanes. Which
//! connection reaches the fake Herdr first is a scheduling accident, so the
//! fixture serves every connection concurrently and decides by the request it
//! carries, never by arrival order.
...
/// How the fake Herdr treats an `agent.prompt` aimed at one pane.
enum PromptPolicy {
    /// Answer `agent_prompted` for that pane.
    Respond,
    /// Check the envelope, report `signal` on the parsed socket, then withhold
    /// the response until the daemon dies without writing anything more.
    Withhold {
        parsed_socket: PathBuf,
        signal: &'static str,
        expect: &'static str,
        forbid: Option<&'static str>,
    },
    /// No prompt bytes may reach this pane: the daemon dies first.
    Unwritten,
}

/// A fake Herdr that serves each connection on its own thread.
///
/// Startup requests (`ping`, `session.snapshot`) are answered wherever they
/// appear. Prompts are dispatched by target pane through `PromptPolicy`. A
/// connection that closes without a byte is a lease the fault point stopped
/// before its write, which is exactly what the kill tests are proving, so it
/// is not an error on its own; `Unwritten` is proven by no prompt to that pane
/// ever parsing.
struct FakeHerdr {
    stop: Arc<AtomicBool>,
...
impl FakeHerdr {
    /// Serve the seeded fleet: `waiting` on `w1:p1`, `owing` on `w1:p2`.
    fn spawn(socket: &Path, waiting: PromptPolicy, owing: PromptPolicy) -> Self {
        let listener = UnixListener::bind(socket).expect("bind fake Herdr");
        listener
            .set_nonblocking(true)
            .expect("nonblocking fake Herdr listener");
        let policies: Arc<HashMap<String, PromptPolicy>> = Arc::new(HashMap::from([
            ("w1:p1".to_string(), waiting),
            ("w1:p2".to_string(), owing),
        ]));
        ...
        let acceptor = thread::spawn(move || {
            ...
            loop {
                match listener.accept() {
                    Ok((stream, _)) => {
                        ...
                        handlers.push(thread::spawn(move || serve_connection(stream, &policies)));
                    }
            ...
            for handler in handlers {
                if let Err(payload) = handler.join() {
    ...
    /// Stop accepting and surface every handler's assertions.
    ///
    /// Call only after the daemon has been reaped: nothing can connect after
    /// that, so the backlog drained before the stop flag is honoured is the
    /// complete set of connections the daemon ever opened.
    fn finish(self) {
        self.stop.store(true, Ordering::SeqCst);
        self.acceptor.join().expect("fake Herdr fixture");
    }
}

fn pane_identity(pane: &str) -> (&'static str, &'static str) {
    match pane {
        "w1:p1" => ("waiting", "term-waiting"),
        "w1:p2" => ("owing", "term-owing"),
        other => panic!("prompt to a pane the seed never declared: {other}"),
    }
}

fn serve_connection(mut stream: UnixStream, policies: &HashMap<String, PromptPolicy>) {
    let mut line = String::new();
    BufReader::new(stream.try_clone().expect("clone stream"))
        .read_line(&mut line)
        .expect("read Herdr request");
    ...
    if !line.ends_with('\n') {
        // Torn by the kill mid-write: the daemon's own record of it is
        // `unknown`, and that is what the recovery assertions check. Only a
        // pane the fault point was holding back must never appear in it.
        for (pane, policy) in policies {
            assert!(
                !(matches!(policy, PromptPolicy::Unwritten)
                    && line.contains(&format!("\"target\":\"{pane}\""))),
                "agent.prompt bytes to {pane} crossed the fault point"
            );
        }
        return;
    }
    ...
    let result = match method.as_str() {
        ...
        "agent.prompt" => {
            ...
            match policies.get(&target) {
                None => panic!("agent.prompt to a pane with no policy: {target}"),
                Some(PromptPolicy::Unwritten) => {
                    panic!("agent.prompt bytes to {target} crossed the fault point")
                }
                Some(PromptPolicy::Respond) => {
                    let (name, terminal) = pane_identity(&target);
                    serde_json::json!({
                        "type":"agent_prompted",
                        "agent":observation(name, &target, terminal)
                    })
                }
                Some(PromptPolicy::Withhold {
                    parsed_socket,
                    signal,
                    expect,
                    forbid,
                    ...
                    assert!(
                        envelope.contains(expect),
                        "{target} envelope lacks {expect:?}: {envelope}"
                    );
                    if let Some(forbid) = forbid {
                        assert!(
                            !envelope.contains(forbid),
                            "{target} envelope carries {forbid:?}: {envelope}"
                        );
                    }
                    UnixStream::connect(parsed_socket)
                        .expect("connect parsed signal")
                        .write_all(format!("{signal}\n").as_bytes())
                        .expect("report parsed prompt");
    ...
    // The daemon may already be dead when this response goes out; the kill
    // tests prove what it recorded, not whether it lived to read the answer.
...
fn kill_after_cancellation_submitted_settles_obligation_without_resend() {
    ...
    let first_herdr = FakeHerdr::spawn(
        &herdr_socket,
        PromptPolicy::Unwritten,
        PromptPolicy::Respond,
    );
...
fn kill_after_cancellation_write_recovers_unknown_without_resend() {
    ...
    let first_herdr = FakeHerdr::spawn(
        ...
        PromptPolicy::Withhold {
            parsed_socket: parsed_socket.clone(),
            signal: "cancellation parsed",
            expect: "<kelpie-system cancellation",
            forbid: None,
        },
        PromptPolicy::Respond,
    );
...
fn kill_after_cancellation_response_recovers_unknown_without_resend() {
    ...
    let (waiting, _owing, ask_message_id) = seed_open_ask(&database);
    let fault_listener = UnixListener::bind(&fault_socket).expect("bind fault harness");
    let first_herdr = FakeHerdr::spawn(&herdr_socket, PromptPolicy::Respond, PromptPolicy::Respond);
    let mut first_daemon = spawn_kelpied(
        &database,
        &kelpie_socket,
        &herdr_socket,
...
fn kill_after_owing_cancellation_write_recovers_unknown_without_resend() {
    ...
    let first_herdr = FakeHerdr::spawn(
        ...
        PromptPolicy::Withhold {
            parsed_socket: parsed_socket.clone(),
            signal: "owing cancellation parsed",
            expect: "<kelpie-system cancellation owing=",
            forbid: Some("reply-to"),
        },
    );
    let mut first_daemon = spawn_kelpied(
        &database,
