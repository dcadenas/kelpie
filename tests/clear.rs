...
fn request_after_connect(
    ...
    // Signal only once the whole line is in the socket: the caller runs a
    // single poll on it, and a poll that accepts before the bytes arrive
    // parks the client for a later pass the test never makes.
